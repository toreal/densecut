...
#define CV_CMP_GT   1
	...
	CmBOOL CmFile::MkDir(CStr& _path)
	{
		if (_path.size() == 0)
			return FALSE;
...
#pragma omp parallel for
	...
	{
		Mat mask = imread(fileName, cv::ImreadModes::IMREAD_GRAYSCALE);
		CV_Assert_(mask.data != NULL, ("Can't find mask image: %s", _S(fileName)));
		compare(mask, 128, mask, CV_CMP_GT);
		return mask;
	}

	CmBOOL CmFile::Move2Dir(CStr& srcW, CStr dstDir)
	{
		vecS names;
		string inDir;
	...
	CmBOOL CmFile::Copy2Dir(CStr& srcW, CStr dstDir)
	{
		vecS names;
		string inDir;
		int fNum = CmFile::GetNames(srcW, names, inDir);
