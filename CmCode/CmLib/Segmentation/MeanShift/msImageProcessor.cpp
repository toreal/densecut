...
int msImageProcessor::GetLabels(int **labels_out)
{
	//check to see if output has been defined for the given input image...
	if(class_state.OUTPUT_DEFINED == false)
		return -1;
...
int msImageProcessor::MyGetRegions(int *myLabels)
{
	if (myLabels!=NULL)
	{
		for (int i=0; i<L; i++)
...
void msImageProcessor::NonOptimizedFilter(float sigmaS, float sigmaR)
{
	...
	for(i = 0; i < L; i++)
	{
		...
		iterationCount = 1;
		while((mvAbs >= EPSILON_E)&&(iterationCount < LIMIT))
		{
...
void msImageProcessor::DefineBoundaries( void )
{
	...
	if((boundaryMap == NULL) ||(boundaryCount == NULL))
...
int MeanShiftEdison(CMat &_img3b, Mat &idx1i, int sigmaS, float sigmaR, int minReg)
{
	...
	for (int i = 0, step = 3*img3b.cols; i < img3b.rows; i++)
		memcpy(imgData + i * step, img3b.ptr<Vec3b>(i), step);
	Mat regIdx1i(img3b.size(), CV_32S);
	mss.DefineImage(imgData, COLOR, img3b.rows, img3b.cols);
	mss.Segment(sigmaS, sigmaR, minReg, HIGH_SPEEDUP);
	int regNum = mss.MyGetRegions((int*)regIdx1i.data);
	delete imgData;
	idx1i = regIdx1i;
	return regNum;
}
